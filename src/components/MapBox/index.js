...
import {ButtonsMap, MenuMap, ShowInfoMap, ShowPhoto} from "..";
...
import {selectedItemMenu, toggleItemMenu} from "../../redux/actions/layoutActions";
import {setFromMap, setPoint} from "../../redux/actions/projectActions";
import {deleteAccess, deleteMarking, deleteSpan, fetchSpans, getSpan, setSpan, setStructures} from "../../redux/actions/spanActions";
import {deleteStructure, fetchStructures, getStructure} from "../../redux/actions/structureActions";
import {deleteSubstation, getSubstations} from "../../redux/actions/substationActions";
...
const MapBox = ({...props}) => {
	...
	const [main, setMain] = useState({width: 0, height: 0});
	...
	function obtenerCoordenadas(photo, main) {
		//console.log("props.structures=",props.structures);
		let dimension = {width: document.documentElement.clientWidth - 200, height: document.documentElement.clientHeight - 200};
		//console.log("dimension=", dimension);
		let imaen = {width: 0, height: 0};

		//caso 1:
		if (main.width < dimension.width) {
			imaen.width = main.width;
		} else {
			imaen.width = dimension.width;
		}
		imaen.height = (imaen.width / main.width) * main.height;
		if (imaen.height <= dimension.height) {

		} else {
			if (main.height < dimension.height) {
				imaen.height = main.height;
			} else {
				imaen.height = dimension.height;
			}
			imaen.width = (imaen.height / main.height) * main.width;
			if (imaen.height <= dimension.height) {

			} else {
				alert('Selecione Otra imagen!');
			}
		}
		//console.log("imaen=", imaen);

		let datos = [];
		let coord_det = {url: "", x: 0, y: 0, category_name: "", item: ""};

		categories.forEach((category) => {
			coord_det.category_name = category.name;
			marker.properties.items.filter(
				({category_id}) => category_id === category.id
			).map((item) => {
				coord_det.item = item.item_parent.name;
				item.deficiencies.map((d) => {
					d.photos.map((p) => {
						if (p.structure_photo.photo == photo) {
							coord_det.url = p.photo;
							let coordenadas = p.photo.split("__LL__");
							if (coordenadas.length === 4) {
								coord_det.x = (imaen.width / main.width) * coordenadas[1];
								coord_det.y = (imaen.height / main.height) * coordenadas[2];
							} else {
								coord_det.x = 0;
								coord_det.y = 0;
							}
							datos.push(Object.assign({}, coord_det));
						}
					});
				});
			});
	...
	function createMap() {
		const coordinates = props.structures.concat(props.spans).concat(props.access).concat(props.markings).concat(props.interactions).concat(
	...
	async function getLayers() {
		...
		let features = await Promise.all(
			props.spans.filter(({id}) => id !== (marker ? marker.properties.id : 0)) // FOR HIDE SPAN SELECTED FOR DELETE
			.map(async (span) => {
		...
		features.forEach((marker) => {
			...
			el.addEventListener("click", (e) => {
				...
				if (itemValueRef.current === 2) {
					...
					formatterImagePrincipal(marker);
					setMarker(marker);
					setItem(el);
					setOpen(true);
				}
	...
	function formatterImagePrincipal(marker) {
		//console.log("marker22=",marker);
		let structure_photo = [];
		marker.properties.items.forEach((item) => {
			item.deficiencies.forEach((def) => {
				def.photos.forEach((p) => {
					if (p) structure_photo.push(p.structure_photo);
				});
			});
		});
		//console.log("structure_photo=",structure_photo);

		let structure_photo_url = [];
		let aux3 = structure_photo.filter((str_p) => {
			if (!structure_photo_url.includes(str_p.photo)) {
				structure_photo_url.push(str_p.photo);
				return true;
			} else return false;
		});
		//console.log("structure_photo_url=",structure_photo_url);
		setImagePrincipal(aux3);
	}
			...
			<div id="map" style={{height: "100%", width: "100%"}}>
				...
				<ShowInfoMap
					...
					showPhoto={(photo) => {
						...
						img.src = photo;
						img.onload = function () {
							setCoord(obtenerCoordenadas(photo, {width: this.width, height: this.height}));
						};
						setOpenPhoto(true);
						setUrl(photo);
					}}
